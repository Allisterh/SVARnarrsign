  ...
  for (i in 1:nvar){
    ...
    plot_list[[i]] <- local({
      ...
      ggplot(data = data.frame(horizon = 1:dim(impt)[2], median = impt[i,,1]), aes(x=horizon, y = median)) + ylim(0,100)+
        geom_ribbon(aes(ymin = impt[i,,2] , ymax = impt[i,,3]), alpha = 0.4, fill = "#F8766D"  )  + geom_line(col = "#F8766D",size=1.5) +
        ylab(varnames[i]) + geom_line(aes(x = horizon, y = rep(0,length(horizon)))) 
    })
  }
